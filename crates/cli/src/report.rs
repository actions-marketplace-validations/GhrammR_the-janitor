...
/// Transliterates common Latin diacritics to their ASCII base letters and
/// replaces any remaining non-ASCII codepoints with `?`.
///
/// Applied to all user-supplied string fields before they are written into
/// the Markdown/LaTeX pipeline so that pdflatex never encounters a codepoint
/// outside its loaded font encoding.  ASCII-only inputs are returned without
/// allocation.
...
pub fn render_markdown(data: &ReportData, repo_name: &str) -> String {
    ...
    out.push_str(&format!(
        "**Repository**: `{}`\n\n",
        html_escape(&sanitize_name(repo_name))
    ));
    out.push_str(&format!(
        "**Total PRs analyzed**: {}\n\n",
        data.entries.len()
    ...
    if !data.sloppiest_users.is_empty() {
        ...
        for (i, u) in data.sloppiest_users.iter().enumerate() {
            out.push_str(&format!(
                "| {} | `{}` | **{}** | {} | {} |\n",
                i + 1,
                html_escape(&trunc_author(&sanitize_name(&u.author), 20)),
                u.total_slop_score,
                u.total_pr_count,
                u.clean_pr_count,
            ));
    ...
    if data.slop_top_indices.is_empty() {
        ...
        for (rank, &i) in data.slop_top_indices.iter().take(10).enumerate() {
            ...
            let author = html_escape(&trunc_author(
                &sanitize_name(e.author.as_deref().unwrap_or("-")),
                20,
            ));
            out.push_str(&format!(
                "| {} | {} | {} | **{}** | {} |\n",
    ...
    if has_findings {
        ...
        for &i in &data.slop_top_indices {
            ...
            let pr = e
                ...
                .map(|n| format!("#{n}"))
                .unwrap_or_else(|| "-".to_owned());
            let author = html_escape(&sanitize_name(e.author.as_deref().unwrap_or("-")));
            out.push_str(&format!("- **PR {pr}** (`{author}`):\n"));
            for (desc, count) in group_strings(&e.antipatterns) {
                let desc_s = sanitize_name(desc);
                if count > 1 {
    ...
    if data.clone_pairs.is_empty() {
        ...
        for (a, b) in display_pairs {
            ...
            let auth_a = html_escape(&trunc_author(
                &sanitize_name(ea.author.as_deref().unwrap_or("unknown")),
                20,
            ));
            let auth_b = html_escape(&trunc_author(
                &sanitize_name(eb.author.as_deref().unwrap_or("unknown")),
                20,
            ));
            out.push_str(&format!(
    ...
    if !data.zombie_indices.is_empty() {
        ...
        for &i in data.zombie_indices.iter().take(LIST_DISPLAY_CAP) {
            ...
            let author = html_escape(&sanitize_name(e.author.as_deref().unwrap_or("unknown")));
    ...
    for repo in &data.repos {
        ...
        if repo.top_sloppiest.is_empty() {
            ...
            for (pr_num, score, state, author) in &repo.top_sloppiest {
                ...
                let author_s = sanitize_name(author);
